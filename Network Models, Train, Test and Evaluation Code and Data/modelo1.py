...
* FICHERO : modelo1.py
* FECHA : Mayo de 2021
* DESCRIPCION: Modelo 0: versión de la red entrenada con un dataset sencillo para realizar una clasificación binaria entre SNR y AF
"""
...
class ECG_SubDataset(Dataset):
  ...
  def __init__(self, datos):
    # data loading
    path = '/scratch/alumno5/work/datos_entreno/SubsetNSRvsAF'
...
# Comprobación del funcionamiento de la carga de datos

#dataset = ECG_SubDataset()
...
# Definición del modelo de red, en este caso CardioNet2 (ya que CardioNet1 no tenía BatchNorm ni Dropout) Batch de dimensiones 32x15x10x750
import numpy as np
...
class CardioNet2(torch.nn.Module):
  def __init__(self):
    ...
    self.lin2 = torch.nn.Linear(512, 2, bias=True)
    self.batchN5 = torch.nn.BatchNorm1d(2)
...
from sklearn.model_selection import train_test_split
files = []
with open('/scratch/alumno5/work/datos_entreno/SubsetNSRvsAF/NombresFicheros.txt', 'r') as texto:
...
path = '/home/alumnos/alumno5/work/Modelos/modelo1/checkpoint0.pt'
...
plt.xlabel('Epoch')
plt.ylabel('Coste')
plt.savefig('/home/alumnos/alumno5/work/Modelos/modelo1/costes.png')
plt.close()
...
plt.savefig('/home/alumnos/alumno5/work/Modelos/modelo1/precisiones_test50epochs.png')
