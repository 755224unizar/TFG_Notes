...
* NIA: 755224
* FICHERO : modelo4_lin3_2.py
* FECHA : Mayo de 2021
* DESCRIPCION: Modelo 3: combina los Modelos 1 y 2, por lo que añade una capa convolucional con stride, una capa lineal despues de esta, 
               y otra capa lineal al final de todas, se consideran las clases concurrentes. Entrenado con el subset de 4 patologías ampliado.
"""
...
# Modelo con dos capas lineales y una capa convolucional más, para el subset de 4 patologías ampliado      Batch de dimensiones 32x15x10x750
import numpy as np
...
class CardioNet2(torch.nn.Module):
  def __init__(self):
    ...
    self.conv4 = torch.nn.Conv2d(128, 256, kernel_size=3, padding=1, stride=(1,2))
    self.batchNC4 = torch.nn.BatchNorm2d(256)

    self.drop = torch.nn.Dropout(0.5)

    self.lin1_0 = torch.nn.Linear(256*6, 1024, bias=True)
    self.batchNL1_0 = torch.nn.BatchNorm1d(1024)
    self.lin1 = torch.nn.Linear(1024, 512, bias=True)
    self.batchNL1 = torch.nn.BatchNorm1d(512)
    self.lin2 = torch.nn.Linear(512, 128, bias=True)
    self.batchNL2 = torch.nn.BatchNorm1d(128)
    self.lin3 = torch.nn.Linear(128, 4, bias=True)
    self.batchNL3 = torch.nn.BatchNorm1d(4)
  ...
  def forward(self, x):
    ...
    x = self.drop(x)
    #print(x.shape)
    x = torch.reshape(x, (B, 256*6))  # Elimino la última dimensión con un reshape
    #print(x.shape)
    x = F.relu(self.batchNL1_0(self.lin1_0(x)))  
    x = F.relu(self.batchNL1(self.lin1(x)))    
    #print(x.shape)
    x = F.relu(self.batchNL2(self.lin2(x)))
    x = self.batchNL3(self.lin3(x))
    #print(x.shape)
...
path = '/home/alumnos/alumno5/work/Modelos/modelo4_capas/modelo4_lin3_2_20ep.pt'
...
plt.xlabel('Epoch')
plt.ylabel('Coste')
plt.savefig('/home/alumnos/alumno5/work/Modelos/modelo4_capas/lin3_2/costes_20ep.png')
plt.close()
